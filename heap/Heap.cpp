...
static size_t heapSizeForHint(HeapSize heapSize)
{
	...
	//+EAWebKitChange
	//10/24/2011 - Commented out the original code which allowed the heap usage to grow to about 16 MB. Added code to 
	// support application specified watermark.
	if(heapSize == LargeHeap)
	{
		size_t heapSizeBytes = JSGetHeapWatermark();
		if(heapSizeBytes < smallHeapSize)// Avoid accidental slow down with 
		{
			ASSERT(false);//Too small heap watermark. Using smallHeapSize
			heapSizeBytes = smallHeapSize;
		}
