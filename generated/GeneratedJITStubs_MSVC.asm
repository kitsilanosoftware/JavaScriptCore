...
cti_op_check_has_instance ENDP
    ...
    EXPORT cti_optimize_from_loop
    IMPORT JITStubThunked_optimize_from_loop
cti_optimize_from_loop PROC
    str lr, [sp, #64] ; sync with THUNK_RETURN_ADDRESS_OFFSET
    bl JITStubThunked_optimize_from_loop
    ldr lr, [sp, #64] ; sync with THUNK_RETURN_ADDRESS_OFFSET
    bx lr
cti_optimize_from_loop ENDP

    EXPORT cti_optimize_from_ret
    IMPORT JITStubThunked_optimize_from_ret
cti_optimize_from_ret PROC
    str lr, [sp, #64] ; sync with THUNK_RETURN_ADDRESS_OFFSET
    bl JITStubThunked_optimize_from_ret
    ldr lr, [sp, #64] ; sync with THUNK_RETURN_ADDRESS_OFFSET
    bx lr
cti_optimize_from_ret ENDP
